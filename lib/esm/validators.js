const validators = {
  ...
  max: {
    exe({
      ...
      return !Array.isArray(received) && !isNaN(received) ? received && parseFloat(received) > max : received.length && received.length > max;
    }
  ...
  min: {
    exe({
      ...
      return !Array.isArray(received) && !isNaN(received) ? received && parseFloat(received) < min : received.length && received.length < min;
    }
  ...
  required_if: {
    exe({
      ...
      if (Array.isArray(other_value)) {
        return !received.length && other_value.some(val => new RegExp(`^${parameter_value.trim()}$`).test(val));
      }

      return !received.length && new RegExp(`^${parameter_value.trim()}$`).test(other_value);
    }
