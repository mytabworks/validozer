...
export default class Validozer {
  ...
  static rulesUpdateMessage(name, message) {
    ...
    messages[name] = is.obj(message) && ['min', 'max'].includes(name) ? { ...messages[name],
      ...message
    } : message;
  }
  ...
  static validate(received, rules, attribute, data) {
    ...
    const isInvalid = array_rules.some(validation => {
      ...
      catch_param = param;
      catch_value = value;
      if (!validators[name]) throw new TypeError(`Validozer does not recognize rule \`${name}\` in \`${attribute}\``);
    ...
    if (isInvalid || validators[catch_name].allowMessageEvenValid) {
      ...
      if (catch_param) {
        ...
        message = message && message.replace(`:${catch_name}`, (catch_param || '').replace(/,/g, ', '));
      }
  ...
  fails() {
    ...
    this.messages = Object.keys(this.rules).map(name => {
      ...
      const validate = Validozer.validate(this.data[name], rules, label || name, this.data);
